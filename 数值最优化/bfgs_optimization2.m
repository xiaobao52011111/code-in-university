function bfgs_optimization2()
    fprintf('优化问题 (ii):\n')
    x0_ii = [0; 1]; % 初始点
    B0_ii = eye(2); % 初始矩阵 B0
    [x_min_ii, f_min_ii, iter_ii] = bfgs(@f2, @grad_f2, x0_ii, B0_ii);
    fprintf('最小值 x: [%f; %f], 最小值 f(x): %f, 迭代次数: %d\n', x_min_ii(1), x_min_ii(2), f_min_ii, iter_ii)
end
